...
class Student {
    ...
    get getGemiddelde(): number {
        if (this._cijfers.length == 0) return 0;
        const som = this._cijfers.reduce((huidigeOptelling, huidigGetal) => huidigeOptelling + huidigGetal );
        return parseFloat((som / (this._cijfers.length)).toFixed(2));
    }

    set setNaam(value: string) {
        if (value.trim() === "") {
